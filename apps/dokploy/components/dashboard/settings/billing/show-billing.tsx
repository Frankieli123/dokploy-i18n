				...
				<div className="rounded-xl bg-background shadow-md ">
					<CardHeader className="">
						<CardTitle className="text-xl flex flex-row gap-2">
							...
							{tCommon("settings.billing.page.title")}
						</CardTitle>
						<CardDescription>
							{tCommon("settings.billing.page.description")}
						</CardDescription>
					</CardHeader>
						...
						<div className="flex flex-col gap-4 w-full">
							...
							>
								<TabsList>
									<TabsTrigger value="monthly">
										{tCommon("settings.billing.tabs.monthly")}
									</TabsTrigger>
									<TabsTrigger value="annual">
										{tCommon("settings.billing.tabs.annual")}
									</TabsTrigger>
								</TabsList>
							</Tabs>
							{admin?.user.stripeSubscriptionId && (
								<div className="space-y-2 flex flex-col">
									<h3 className="text-lg font-medium">
										{tCommon("settings.billing.plan.title")}
									</h3>
									<p className="text-sm text-muted-foreground">
										{tCommon("settings.billing.plan.usage", {
											servers: servers ?? 0,
									...
									{admin && admin.user.serversQuantity! <= (servers ?? 0) && (
										<div className="flex flex-row gap-4 p-2 bg-yellow-50 dark:bg-yellow-950 rounded-lg items-center">
											...
											<span className="text-sm text-yellow-600 dark:text-yellow-400">
												{tCommon("settings.billing.plan.limitReached")}
											</span>
										</div>
									)}
								</div>
							...
							<div className="flex flex-col gap-1.5 mt-4">
								<span className="text-base text-primary">
									{tCommon("settings.billing.help.title")}
								</span>
								<span className="text-sm text-muted-foreground">
									{tCommon("settings.billing.help.description")}
								</span>
								<Button className="rounded-full bg-[#5965F2] hover:bg-[#4A55E0] w-fit">
									...
									>
										...
										{tCommon("settings.billing.help.joinDiscord")}
									</Link>
								</Button>
							</div>
							{isLoading ? (
												...
												>
													{isAnnual && (
														<div className="mb-4 flex flex-row items-center gap-2">
																<Badge>
																	{tCommon("settings.billing.badge.recommended")}
																</Badge>
														</div>
													)}
													{isAnnual ? (
													...
													>
														{[
															...
															>
																<CheckIcon />
																<span className="ml-4">{tCommon(featureKey)}</span>
															</li>
														))}
													</ul>
													<div className="flex flex-col gap-2 mt-4">
														<div className="flex items-center gap-2 justify-center">
															<span className="text-sm text-muted-foreground">
																{tCommon("settings.billing.serversLabel", {
															...
															{data?.subscriptions?.length === 0 && (
																<div className="justify-end w-full">
																	<Button
																		...
																		disabled={serverQuantity < 1}
																	>
																		{tCommon("settings.billing.button.subscribe")}
																	</Button>
																</div>
															)}
														</div>
