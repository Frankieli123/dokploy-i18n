...
export const ssoProviderBodySchema = z.object({
	...
	domains: z
		.array(z.string())
		.transform((value) =>
			Array.from(new Set(value.map((item) => item.trim().toLowerCase()).filter(Boolean))),
		)
		.refine((value) => value.every((item) => domainRegex.test(item)), {
	...
	overrideUserInfo: z.boolean().optional().default(false),
});
